...
const { banner, banner2 } = require("./funções de cmd/funções/banner")
...
const { state, saveState } = await useSingleFileAuthState(qrcode)
    ...
    const pl = plConnect({
        ...
        browser: ['NEZUKO-BOT','safari','3.0'],
        auth: state
})

pl.ev.on('group-participants.update', async (anu) => {
  try {
      
      require("./bemvindo")(pl, anu)
...
require('./funções de cmd/menu/menu')
  ...
  nocache('./funções de cmd/menu/menu', Módulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(` Módulo "${Módulo}" atualizado`)))
	nocache('./configurar.js', Módulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`Módulo "${Módulo}" atualizado!`)))
	nocache('./nezuko.js', Módulo => console.log(chalk.greenBright('[ WHATSAPP BOT ]  ') + time + chalk.cyanBright(`Módulo "${Módulo}" atualizado!`)))
...
try {
        ...
        require("./nezuko.js")(pl, m, messages, store)
        } catch (err) {
            console.log(err)
        }
    })
...
Lft = await getBuffer(`https://hardianto.xyz/api/goodbye3?profile=${encodeURIComponent(ppuser)}&name=${encodeURIComponent(nama)}&bg=https://telegra.ph/file/d460e086f9f9bf6b04e17.jpg&namegb=${encodeURIComponent(metadata.subject)}&member=${encodeURIComponent(memb)}`)
	           ...
	           const members = metadata.participants.length
                let unicorndoc = {key: {fromMe: false,"participant":"0@s.whatsapp.net", "remoteJid": "916909137213-1604595598@g.us"}, "message": {orderMessage: {itemCount: 666,status: 200, jpegThumbnail: sai_kk, surface: 200, message: `${metadata.subject}`, orderTitle: 'YOSHI', sellerJid: '0@s.whatsapp.net'}}, contextInfo: {"forwardingScore":999,"isForwarded":true},sendEphemeral: true}
                xbody = `
┏━━━━━━[🗯]━━━━━━━━ ➤
┃•「 OLÁ 👋 」
└┬☆ 「 @${Xnome.split("@")[0]}  」
...
let buttons = [
{buttonId: `${prefix}menu`, buttonText: {displayText: '[❤️‍🔥] MENU [❤️‍🔥]'}, type: 1},
{buttonId: `${prefix}menuc`, buttonText: {displayText: '[❤️‍🔥] MENU2 [❤️‍🔥]'}, type: 1}
]
let buttonMessage = {
image: wm_fatih,
jpegThumbnail: sai_kk,
mentions: [num],
caption: xbody,
footer: `${NomeDoBot}`,
buttons: buttons,
...
let buttons = [
{buttonId: `KSKSKS`, buttonText: {displayText: '🥱 FDS, LIGO NÃO'}, type: 1},
{buttonId: `KSKSKS`, buttonText: {displayText: '⚘️ SAIU O CORNO'}, type: 1}
]
let buttonMessage = {
image: wm_fatih,
jpegThumbnail: sai_kk,
...
//===================================== 
   ...
   const {
        ...
        if (connection === 'open') {
            success('2', `『️️ 𝗟𝗢𝗚𝗔𝗗𝗢 𝗖𝗢𝗠 𝗦𝗨𝗖𝗘𝗦𝗦𝗢 』️️`)
        }
